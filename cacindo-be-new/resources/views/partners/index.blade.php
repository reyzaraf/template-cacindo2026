...
@section('content')
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        ...
                        <h4 class="card-title">
                            <a class="add-menu-sidebar" href="{{ route('admin.partners.create') }}">Create New</a>
                        </h4>
                    </div>
                    <div class="card-body">
                            ...
                            <table id="datatable" class="display min-w850">
                                ...
                                <tbody>
                                    @foreach ($partners as $partner)
                                        <tr data-identifier="{{ $partner->id }}">
                                            <td></td>
                                            <td>{{ $partner->title }}</td>
                                            <td><img height="200" src="{{ Storage::disk('public')->url(@$partner->thumbnail) }}" alt=""></td>
                                            <td>
                                                <b>
                                                    {{ $partner->position }}
                                                </b>
                                            ...
                                            {{-- <td>
                                                <img src="{{ Storage::disk('public')->url($partner->image) }}" alt=""
                                            ...
                                            <form action="{{ route('admin.partners.destroy', $partner->id) }}" method="post"
                                                id="destroy-{{ $partner->id }}" style="display: none">
                                                @csrf
                                                @method('DELETE')
                                            </form>
                                        </tr>
    ...
    <form action="{{ route('admin.partners.bulkDelete') }}" id="deleteSelectedForm" method="POST" class="d-none">
        @csrf
        @method('DELETE')
