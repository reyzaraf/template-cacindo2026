                        ...
                        <form action="{{ route('admin.partners.store') }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" id="title" name="title"
                                    placeholder="Slider Title" style="color: #656773;" required>
                                @error('title')
                                    <div class="text-sm text-danger">{{ $message ?? 'Something error' }}</div>
                                @enderror
                            </div>
                            <div class="form-group">
                                <label for="position" class="text-capitalize">Position Slider</label>
                                <input name="position" id="position" rows="10" class="form-control" style="color: #656773;" value="{{ @$partner->position }}" type="number">
                                @error('position')
                                    <div class="text-sm text-danger">{{ $message ?? 'Something error' }}</div>
                                @enderror
                            </div>
