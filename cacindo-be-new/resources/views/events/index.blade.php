...
@section('content')
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        ...
                        <h4 class="card-title">
                            <a class="add-menu-sidebar" href="{{ route('admin.events.create') }}">Create New</a>
                        </h4>
                    </div>
                    <div class="card-body">
                            ...
                            <table id="datatable" class="display min-w850">
                                ...
                                <tbody>
                                    @foreach ($events as $event)
                                        <tr data-identifier="{{ $event->id }}">
                                            <td></td>
                                            <td>{{ $event->title }}</td>
                                            <td>{{ $event->short_description }}</td>
                                            <td><img height="100" src="{{ Storage::disk('public')->url(@$event->thumbnail) }}" alt=""></td>
                                            <td>{{ $event->category }}</td>
                                            <td>{{ $event->created_at->diffForHumans() }}</td>
                                            {{-- <td>
                                                <img src="{{ Storage::disk('public')->url($event->image) }}" alt=""
                                            ...
                                            <form action="{{ route('admin.events.destroy', $event->id) }}" method="post"
                                                id="destroy-{{ $event->id }}" style="display: none">
                                                @csrf
                                                @method('DELETE')
                                            </form>
                                        </tr>
    ...
    <form action="{{ route('admin.events.bulkDelete') }}" id="deleteSelectedForm" method="POST" class="d-none">
        @csrf
        @method('DELETE')
