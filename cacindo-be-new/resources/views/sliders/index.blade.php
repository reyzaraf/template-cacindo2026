...
@section('content')
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        ...
                        <h4 class="card-title">
                            <a class="add-menu-sidebar" href="{{ route('admin.sliders.create') }}">Create New</a>
                        </h4>
                    </div>
                    <div class="card-body">
                            ...
                            <table id="datatable" class="display min-w850">
                                ...
                                <tbody>
                                    @foreach ($sliders as $slider)
                                        <tr data-identifier="{{ $slider->id }}">
                                            <td></td>
                                            <td>{{ $slider->title }}</td>
                                            <td><img height="200" src="{{ Storage::disk('public')->url(@$slider->thumbnail) }}" alt=""></td>
                                            <td>
                                                <b>
                                                    {{ $slider->position }}
                                                </b>
                                            ...
                                            {{-- <td>
                                                <img src="{{ Storage::disk('public')->url($slider->image) }}" alt=""
                                            ...
                                            <form action="{{ route('admin.sliders.destroy', $slider->id) }}" method="post"
                                                id="destroy-{{ $slider->id }}" style="display: none">
                                                @csrf
                                                @method('DELETE')
                                            </form>
                                        </tr>
    ...
    <form action="{{ route('admin.sliders.bulkDelete') }}" id="deleteSelectedForm" method="POST" class="d-none">
        @csrf
        @method('DELETE')
