...
@section('content')
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        ...
                        <h4 class="card-title">
                            <a class="add-menu-sidebar" href="{{ route('admin.pages.create') }}">Create New</a>
                        </h4>
                    </div>
                    <div class="card-body">
                            ...
                            <table id="datatable" class="display min-w850">
                                ...
                                <tbody>
                                    @foreach ($pages as $page)
                                        <tr data-identifier="{{ $page->id }}">
                                            <td></td>
                                            <td>{{ $page->title }}</td>
                                            <td>{{ $page->created_at->diffForHumans() }}</td>
                                            {{-- <td>
                                                <img src="{{ Storage::disk('public')->url($page->image) }}" alt=""
                                            ...
                                            <form action="{{ route('admin.pages.destroy', $page->id) }}" method="post"
                                                id="destroy-{{ $page->id }}" style="display: none">
                                                @csrf
                                                @method('DELETE')
                                            </form>
                                        </tr>
    ...
    <form action="{{ route('admin.pages.bulkDelete') }}" id="deleteSelectedForm" method="POST" class="d-none">
        @csrf
        @method('DELETE')
