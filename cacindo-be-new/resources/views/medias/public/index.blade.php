    ...
    <div class="content-wrapper oh">
        ...
        <section class="section-wrap bottom-divider">
            <div class="container">
                        ...
                        <div class="col-lg-4">
                            <article class="entry card box-shadow hover-up" style="height: 100%;margin-bottom: 5px;">
                                <div class="entry__img-holder card__img-holder">
                                    <a href="{{ route('medias.showPublic', $media->id) }}">
                                        <img src="{{ Storage::disk('public')->url($media->thumbnail) }}" class="entry__img"
                                            alt="" style="height: 20rem;object-fit: cover;" />
                                    </a>
                                    <div class="entry__date">
                                        <span class="entry__date-day">{{ $media->created_at->format('d') }}</span>
                                        <span class="entry__date-month">{{ $media->created_at->format('M') }}</span>
                                    </div>
                                    <div class="entry__body card__body">
                                        <h4 class="entry__title text-green">
                                            <a href="{{ route('medias.showPublic', $media->id) }}">{{ $media->title }}</a>
                                        </h4>
                                        <div class="entry__excerpt">
                                            <p
                                                style="display: -webkit-box;-webkit-line-clamp: 3;-webkit-box-orient: vertical;overflow: hidden;">
                                                {{ $media->short_description ?? '-' }}</p>
                                        </div>
                                    </div>
                                </div>
                            </article>
                ...
                {{ $medias->onEachSide(2)->links('pagination::default') }}
