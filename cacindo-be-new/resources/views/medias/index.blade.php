...
@section('content')
    <div class="container-fluid">
        ...
        <div class="page-titles">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active"><a href="javascript:void(0)">Media</a></li>
            </ol>
        </div>
        <!-- row -->
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        ...
                        <h4 class="card-title">
                            <a class="add-menu-sidebar" href="{{ route('admin.medias.create') }}">Create New</a>
                        </h4>
                    </div>
                    <div class="card-body">
                            ...
                            <table id="datatable" class="display min-w850">
                                ...
                                <tbody>
                                    @foreach ($medias as $media)
                                        <tr data-identifier="{{ $media->id }}">
                                            <td></td>
                                            <td>{{ $media->title }}</td>
                                            <td>{{ $media->short_description }}</td>
                                            <td><img height="200" src="{{ Storage::disk('public')->url(@$media->thumbnail) }}" alt=""></td>
                                            <td>{{ $media->category }}</td>
                                            <td>{{ $media->created_at->diffForHumans() }}</td>
                                            {{-- <td>
                                                <img src="{{ Storage::disk('public')->url($media->image) }}" alt=""
                                            ...
                                            <form action="{{ route('admin.medias.destroy', $media->id) }}" method="post"
                                                id="destroy-{{ $media->id }}" style="display: none">
                                                @csrf
                                                @method('DELETE')
                                            </form>
                                        </tr>
    ...
    <form action="{{ route('admin.medias.bulkDelete') }}" id="deleteSelectedForm" method="POST" class="d-none">
        @csrf
        @method('DELETE')
